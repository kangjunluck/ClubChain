...
import com.blockback.init.common.jwt.JwtTokenUtil;
...
public class UserController {
    ...
    public ResponseEntity<UserResponse> login(@RequestBody @ApiParam(value="로그인 정보", required = true) UserLoginReq loginInfo, @ApiParam(hidden = true) HttpSession session) {
        ...
        if (user == null)
        {
            return ResponseEntity.status(404).body(UserResponse.of(401, "존재하지 않는 계정입니다."));
        }
        if(passwordEncoder.matches(password, user.getPassword())) {
            JwtTokenUtil.getToken(userEmail);
            // 세션에 저장해준다. 무엇을 저장해야할까.??
            session.setAttribute("signedUser", userEmail);
