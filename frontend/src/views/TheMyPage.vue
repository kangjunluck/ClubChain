    ...
    <div class="container d-flex flex-column justify-content-center align-items-center">
      ...
      <div class="row w-50">
        <div class="d-flex justify-content-between align-items-center mb-3 p-0">
          <img :src="selecturl" alt="" class="px-0" style="width:3rem; height:3rem;" />
          <div class="w-50">
            <p class="m-0 h5">{{ this.userinfo.userEmail }}</p>
            <p class="m-0 h5">{{ this.userinfo.usernickname }}</p>
          </div>
        </div>
        <button @click="profileUpdate" class="btn update-button w-50">설정</button>
      </div>
      <hr size="1px" width="60%">
      <div class="row w-50">
        <div class="p-0 mb-1" style="text-align:left;" @click="myClubInfo">가입한 동호회</div>
